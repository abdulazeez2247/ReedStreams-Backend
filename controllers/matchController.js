...
const Stream = require("../models/stream"); // ✅ import stream model
const API_BASE_URL = "https://api.thesports.com";
const USER_KEY = process.env.THE_SPORTS_API_USER;
const SECRET_KEY = process.env.THE_SPORTS_API_SECRET;
...
// ============================
const getproxyStream = async (req, res, next) => {
  // ... (your existing proxy code unchanged)
};
...
// ============================
const getLiveStreams = async (req, res, next) => {
  // ... (your existing getLiveStreams code unchanged)
};

// ============================
// LOG STREAM PLAY
// ============================
const logStreamPlay = async (req, res, next) => {
  try {
    const { streamId, matchId, url } = req.body;
    if (!streamId) return res.status(400).json({ error: "streamId required" });

    // Find stream and increment playCount
    ...
    res.status(500).json({ error: "Failed to log stream play", details: err.message });
  }
};

// ============================
// SINGLE MATCH DIARY (stub)
// ============================
const getSingleMatchDiary = async (req, res, next) => {
  const { sportName, matchId } = req.params;
...
module.exports = {
  ...
  getSingleMatchDiary,
  getproxyStream,
  logStreamPlay, // ✅ export new function
};
